...
type
    MaterialTemplate* = ref object of RootObj # Material Template
        name* : string
        Ka*, Kd*, Ks*, Tf* : Color3
        d*     : float
        illum* : int
    ...
    MatTmplLib* = ref object of RootObj # Material Template Library
        mtlPathFile: string
        mtls : Table[string, MaterialTemplate]


proc `$`*(o:MaterialTemplate): string =
...
proc newMaterialTemplate(name:string): MaterialTemplate =
    result = new MaterialTemplate
    result.name = name
    result.Ka = WHITE
    result.Kd = WHITE
    result.Ks = BLACK

proc parseMaterialTemplateFile(mtlPath, mtlFileName :string): MatTmplLib =
    ...
    #result.mtls = initTable[string, MaterialTemplate]() # () is important = dict()

    result.mtlPathFile = mtlPathFile
    # echo fmt"parseMaterialTemplateFile from {result.mtlPathFile}"
    ...
    for i, rawLine in pairs(mtlLines):
        ...
        if cmd == "newmtl":
            name = tokens[0]
            result.mtls[name] = newMaterialTemplate(name)
        elif len(line) >= 6:
...
type
    ...
    Group = ref object of RootObj
        ...
        uvtxs   : seq[Vec2f]
        couls   : seq[Vec3f]
        usemtl  : MaterialTemplate

    ObjsManager* = ref object of RootObj
        objLdr : ObjLoader
        groups : Table[string, Group]
        oGrps, gGrps : seq[Group]
        debugTextureFile: string
        ignoreTexture : bool
        debugUvtxs : seq[Vec2f]
        vunIdxsFastList: SeqIdx3FastFind
        allFaceGroupNames: seq[string]
        selGrp: Group # Option[Group] # set: some(x) or none(x); test: x.isNone x.isSome; get x.get()
    ...
    ObjLoader* = ref object of RootObj
        ...
        allUvtxs   : seq[Vec2f]
        allCouls   : seq[Vec3f]
        matTplLib  : MatTmplLib # Option[MatTmplLib]
        lineIdx    : int
        errorsCount: int
        key        : string
        tokens     : seq[string]
...
proc `$`*(self: Group): string =
    ...
    result = fmt"Group:{self.name:19}: {self.typ}, debug:{self.debug}, check:{self.check:5}, ignoreTexture:{self.ignoreTexture:5}, debugTextureFile:'{self.debugTextureFile}'"
    if self.usemtl == nil:
        result &= ", usemtl is None"
    else:
        result &= fmt", usemtl:{self.usemtl.name:19}"
...
proc check_v_u_n_idxs(self: Group; vunIdxs: array[3, uint32]) =
    if not self.check : return

    let vidx = vunIdxs[0]
    let uidx = vunIdxs[1]
    let nidx = vunIdxs[2]
    assert self.vMin <= vidx and vidx < self.vMax                    , fmt"Group '{self.name}' addFace {self.vMin}<= vidx:{vidx} <{self.vMax} !{NL}{self}"
    if uidx != IdxNone: assert self.uMin <= uidx and uidx < self.uMax, fmt"Group '{self.name}' addFace {self.uMin}<= uidx:{uidx} <{self.uMax} !{NL}{self}"
    if nidx != IdxNone: assert self.nMin <= nidx and nidx < self.nMax, fmt"Group '{self.name}' addFace {self.nMin}<= nidx:{nidx} <{self.nMax} !{NL}{self}"
...
proc add_vunIdxs(self: Group; vunIdxs: Idx3): Idx =
    self.check_v_u_n_idxs(vunIdxs)
    result = self.objMgr.vunIdxsFastList.addIfNotYetAndReturnIndex(vunIdxs)
...
proc addGroup(self: ObjsManager; grpTyp:GrpTyp; grpName:string, check:bool) =
    #assert grpTyp in (GrpTyp.oGROUP, GrpTyp.gGROUP)
    ...
    if   grpTyp == GrpTyp.oGROUP : self.oGrps.add(grp)
    elif grpTyp == GrpTyp.gGROUP : self.gGrps.add(grp)
    else: raise
    self.selGrp = grp

proc addObjGroup(self: ObjsManager; grpName="noName", check=true) =
...
proc newObjsManager(parent:ObjLoader; debugTextureFile="", ignoreTexture=false, check=false): ObjsManager =
    ...
    result.groups = initTable[string, Group]()
    #result.oGrps  = list()
    #result.gGrps  = list()
...
proc `$`*(self: ObjsManager): string =
    ...
    for name, grp in pairs(self.groups):
        ...
        result &= fmt"{NL}{name:19}:{grp}"

proc addFaceGroup(self: ObjsManager; grpName="noName", check=true) =
...
proc mergesGroups(self: ObjsManager; groups: seq[string]; debug=0): GroupMerged =
    ...
    for i, name in pairs(groups):
        ...
        grpRng.texIdx = 0.Idx # 0 = no texture, 1, 2 ... no texture
        if not self.ignoreTexture:
            var textureFile = ""

            if grp.debugTextureFile.len > 0 : # string
                if debug >= 2: echo fmt"{grp.debugTextureFile} selected"
                textureFile = grp.debugTextureFile
            elif grp.usemtl != nil:
                #let usemtl = grp.usemtl.get()
                if   grp.usemtl.mapKd != "" : textureFile = grp.usemtl.mapKd
                elif grp.usemtl.mapKa != "" : textureFile = grp.usemtl.mapKa
                elif grp.usemtl.mapKs != "" : textureFile = grp.usemtl.mapKs
                if debug >= 2 and textureFile.len > 0: echo fmt"{textureFile} from usemtl.mapKc selected"
            elif self.objLdr.texPathFile.len > 0:
                textureFile = self.objLdr.texPathFile
                if debug >= 2 : echo fmt"{textureFile} from objLdr.texPathFile selected"

            #echo fmt"mergesGroups: textureFile:'{textureFile}'"
            if textureFile != "": # find if texture is already in list rgTxfil
                ...
                echo fmt"texture for group '{grp.name}' not found in:{NL}{result.rgTxfil.txfis}"
                echo "MESSAGE: use option -notx (no texture) or -dbtx (debug texture) or have in obj file a line: mtllib file.mtl and a line usemtl name"
                quit()

            echo fmt"   merge group {name:19}, texIdx:{grpRng.texIdx:2} : {textureFile}"
            # generate indexes buffer as GLfloat to index textures in fragmtShader

        result.rgTxfil.rangs.add(grpRng)
...
proc parse_mtlib_line(self:ObjLoader) =
    #echo fmt"lin{self.lineIdx:6}:'{self.lineRead}': parse_mtlib_line:"
    ...
    self.matTplLib = parseMaterialTemplateFile(mtlPath, mtlFile)
...
proc parse_fGrp_line(self:ObjLoader) =
    let name = self.tokens[0]
    echo fmt"parse_fGrp_line: '{name}'"
    self.objMgr.addFaceGroup(name)

proc parse_sGrp_line(self:ObjLoader) =
    self.objMgr.selGrp.paramS = self.tokens[0]
    echo fmt"{self.objMgr.selGrp.name}: {self.objMgr.selGrp.paramS}"
...
proc parse_usemtl_line(self:ObjLoader) = # Material Template Library
    ...
    # echo fmt"lin{self.lineIdx:6}:'{self.lineRead}': parse_usemtl_line:"
...
proc parse_face_line(self:ObjLoader) =
    ...
    for i, xyz_uv_n_idxsStr in pairs(faceIdxsStr):
        ...
        #echo "idx3: ", idx3
        let vunIdxsIdx = self.objMgr.selGrp.add_vunIdxs(idx3)
...
const stateChanges: State2Cmd2States =
  {
    ...
    States.IN_VT : {"vt"    : States.NoChg , "vn": States.IN_VN  , "f" : States.IN_F , "g" : States.IN_G , "end": States.END}.toTable,
    ...
    States.IN_F  : {"f"     : States.NoChg , "o" : States.IN_O   , "v" : States.IN_V , "l" : States.IN_L , "end": States.END}.toTable,
    States.IN_F  : {"l"     : States.NoChg , "f" : States.IN_F   , "o" : States.IN_O , "v" : States.IN_V , "end": States.END}.toTable,
    States.IN_G  : {"usemtl": States.USEMTL, "s" : States.IGNORE , "v" : States.IN_V , "f" : States.IN_F , "end": States.END}.toTable,
    States.IN_S  : {"usemtl": States.USEMTL, "s" : States.IGNORE , "f" : States.IN_F , "end": States.END}.toTable,
    States.USEMTL: {"s"     : States.IN_S                        , "f" : States.IN_F , "end": States.END}.toTable
  }.toTable
...
when isMainModule:
    ...
    if model == nil:
      ...
      if not objLoader.loadModel(model,ignoreTexture=ignoreTexture, debugTextureFile=debugTextureFile, check=true, debug=debug, dbgDecountReload=4):
          quit()
      let idxedBufs: IndexedBufs = objLoader.loadOglBufs(grpsToInclude= @[], grpsToExclude= @[], debug=1)
      echo "rangTxFils: ", idxedBufs.rgTxfil
